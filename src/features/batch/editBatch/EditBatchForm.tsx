...
export default function EditBatchForm({ batch, onClose }: Props) {
    ...
    const [capsulesPerUnit, setCapsulesPerUnit] = useState(batch.capsules_per_unit);

    const [recipeId, setRecipeId] = useState<string | null>(batch.recipe_id);
    ...
    return (
        <form className="editBatchForm" onSubmit={handleSubmit}>
            ...
            <select value={recipeId ?? ''} onChange={(e) => setRecipeId(e.target.value || null)}>
                <option value="">Без рецептуры</option>
