    ...
    useEffect(() => {
        fetch(`${import.meta.env.BASE_URL}db.json`)
            ...
            .then((data) => {
                ...
                const ingredients = data.find((t: any) => t.name === 'ingredient')?.data ?? [];

                dispatch(
                    setIngredients(
                        ingredients.map((i: any) => ({
                            ...i,
                            ingredient_stock_qty: Number(i.ingredient_stock_qty),
                        })),
                    ),
                );

                const recipes = data.find((t: any) => t.name === 'recipe')?.data ?? [];

                dispatch(setRecipes(recipes));

                const recipeIngredients =
                    data.find((t: any) => t.name === 'recipe_ingredient')?.data ?? [];
